...
class array:
    ...
    def __getitem__(
            self: array, idx: Union[slice, int, bool]) -> Union[float, int, complex]:
        if isinstance(idx, int):
            ...
            if isinstance(idx[0], bool):
                return array([self._data[i]
                              for i, ind in enumerate(idx) if ind])
    ...
    def __setitem__(
            self: array, idx: Union[slice, int, bool], value: Union[float, int, complex]) -> None:
        if isinstance(idx, int):
            idx = idx if idx >= 0 else idx + self._shape
    ...
    def __add__(self: array, other: Union[int, float, complex,
                                          List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            ...
            return array([self._data[i] + other[i]
                          for i in range(len(self._data))])
    ...
    def __iadd__(self: array, other: Union[int, float, complex,
                                           List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            res = [self._data[i] + other
                   for i in range(len(self._data))]
    ...
    def __sub__(self: array, other: Union[int, float, complex,
                                          List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            res = [self._data[i] - other
                   for i in range(len(self._data))]
    ...
    def __isub__(self: array, other: Union[int, float, complex,
                                           List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            res = [self._data[i] - other
                   for i in range(len(self._data))]
    ...
    def __mul__(self: array, other: Union[int, float, complex,
                                          List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            res = [self._data[i] * other
                   for i in range(len(self._data))]
    ...
    def __imul__(self: array, other: Union[int, float, complex,
                                           List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            res = [self._data[i] * other
                   for i in range(len(self._data))]
    ...
    def __truediv__(
            self: array, other: Union[int, float, complex, List[Union[int, float, complex]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            ...
            assert len(
                other) == self._shape, f"incompatible shapes {len(other)} != {self._shape}"
            return array([self._data[i] * 1 / (other[i])
                          for i in range(len(self._data))])
        elif isinstance(other, List):
            assert len(
                other) == self._shape, f"incompatible shapes {len(other)} != {self._shape}"
    ...
    def __itruediv__(
            self: array, other: Union[int, float, List[Union[int, float]]], /) -> array:
        if isinstance(other, (int, float, complex)):
            res = [self._data[i] * 1 / (other) for i in range(len(self._data))]
            return array(res)
        elif isinstance(other, array):
    ...
    def append(self: array, other: Union[int, float,
                                         complex, List[Union[int, float, complex]]]) -> None:
        if isinstance(other, (int, float, complex)):
    ...
    @property
    def dtype(self: array) -> Union[float, int,
                                    complex]: return type(self._data[0])

    @property
    def data(self: array) -> List[Union[float,
                                        int, complex]]: return self._data

    @property
    def real(self: array) -> List[Union[float, int]
                                  ]: return array([a.real for a in self._data])

    @property
    def imag(self: array) -> List[Union[float, int]
                                  ]: return array([a.imag for a in self._data])
    ...
    def argsort(self: array) -> array:
        return array(sorted(range(len(self._data)),
                            key=self._data.__getitem__))

    def sort(
        self: array) -> array: return array(sorted(self._data, key=lambda x: x.real))
    ...
    def exp(self: array) -> array:
        if self.dtype in (int, float):
            ...
            return array([math.exp(a.real) * (math.cos(a.imag) +
                                              1j * math.sin(a.imag)) for a in self._data])
    ...
    def log(self: array) -> array:
        if self.dtype in (int, float):
            ...
            m = self.modulus()
            argz = self.arg()
            return array([math.log(m[i]) + 1j * argz[i]
                          for i in range(len(m))])

    def dot(self: array, other) -> Union[float,
                                         int, complex]: return sum(self * other)
    ...
    def conjugate(
        self: array) -> array: return array([a.conjugate() for a in self._data])
